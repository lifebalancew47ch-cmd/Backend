...
/// <summary>
/// Endpoints that produce a comprehensive historical report for a family.
/// Membership is validated to prevent unauthorized (IDOR) access.
/// </summary>
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/reports/family")]
...
public sealed class FamilyReportsController : ApiControllerBase
{
    ...
    /// <summary>Initializes a new instance of <see cref="FamilyReportsController"/>.</summary>
    public FamilyReportsController(ICurrentUserService currentUserService)
    {
    ...
    /// <summary>Generates a comprehensive family historical report.</summary>
    [HttpGet("{familyId}")]
    [ProducesResponseType(typeof(ApiResponse<FamilyReportResponse>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetFamilyReport(
        string familyId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return HandleResult(await Mediator.Send(
            new GetFamilyReportQuery(familyId, userId, roles, from, to),
            cancellationToken));
    }
}
