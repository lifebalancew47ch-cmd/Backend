...
5. **Fail-closed (NO SIMULAR DATOS NUNCA)**: DO NOT mock/stub upstream data. Si un servicio upstream o de terceros (ej. SedentaryEngine) falla, el servicio DEBE fallar (arrojar `UpstreamServiceUnavailableException` → 503). ESTÁ ESTRICTAMENTE PROHIBIDO para cualquier IA o desarrollador devolver listas vacías `[]` o datos simulados para "arreglar" un error 500/503. Los errores deben ser visibles. (Excepción única: DashboardService sí tiene graceful degradation autorizado).
